...
│   ├── +html.tsx              # قالب HTML للويب (lang=ar dir=rtl)
│   ├── index.tsx              # الشاشة الرئيسية (بحث + تصنيفات + شبكة منتجات)
│   ├── cart.tsx               # شاشة السلة (الكميات + المجموع)
│   ├── checkout.tsx           # شاشة إتمام الطلب (نموذج بيانات الزبون)
│   ├── order-success.tsx      # شاشة تأكيد إرسال الطلب
│   └── product/
...
1. في **السلة** يضغط الزبون **"إتمام الطلب"** فينتقل إلى شاشة **`checkout`**.
2. في شاشة إتمام الطلب يملأ بياناته: **الاسم** و**العنوان** (إلزاميان) + الهاتف وملاحظات
   (اختياريان)، مع عرض ملخّص الطلب والمجموع.
3. عند الضغط على **"إرسال الطلب عبر واتساب"** يُبنى نص رسالة يتضمّن:
   اسم المتجر، كل منتج (`الاسم × الكمية = السعر الفرعي`)، **المجموع الكلي**،
   ثم **بيانات الزبون** (الاسم/الهاتف/العنوان/الملاحظات).
4. يُرمَّز النص عبر `encodeURIComponent` ويُبنى رابط
   `https://wa.me/<WHATSAPP_NUMBER>?text=<MESSAGE>`، ويُفتح عبر `Linking.openURL`
   (الجوال) أو تبويب جديد (الويب)، ثم تُفرَّغ السلة وتظهر شاشة **التأكيد** `order-success`.

الكود في `lib/whatsapp.ts` (بناء الرسالة) و`app/checkout.tsx` (النموذج).
