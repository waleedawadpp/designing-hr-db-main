...
│   ├── admin.tsx              # لوحة الأدمن لتعديل الأسعار (محمية بـ PIN)
│   └── product/
...
│   └── PricesContext.tsx      # الأسعار الحيّة (دمج السحابة/المحلي فوق الكتالوج)
├── data/
│   └── products.ts            # ★ قائمة المنتجات (id, name, price, category, image)
├── lib/
│   ├── format.ts              # تنسيق الأسعار والعملة
│   ├── whatsapp.ts            # بناء رابط واتساب وفتحه
│   ├── firebase.ts            # تهيئة Firebase (آمنة عند غياب الإعدادات)
│   └── prices.ts              # طبقة الأسعار (Firestore + AsyncStorage)
├── assets/
...
## 🔐 لوحة الأدمن وتعديل الأسعار

التطبيق يحوي شاشة أدمن (`app/admin.tsx`) لتعديل أسعار كل المنتجات من داخل التطبيق.

### كيف تفتحها؟
**اضغط ضغطة مطوّلة (~0.7 ثانية) على شعار المتجر** في رأس الصفحة الرئيسية،
ثم أدخل **رمز الدخول**. الرمز الافتراضي `1234` ويُغيَّر من `config.ts` (المتغيّر `ADMIN_PIN`).

### وضعان للتخزين
| الوضع | متى | أين تُحفظ الأسعار |
|---|---|---|
| **محلي** (افتراضي) | إعدادات Firebase فارغة | على جهاز الأدمن فقط |
| **سحابي** (موصى به للنشر) | بعد ملء إعدادات Firebase | في السحابة، وتتزامن **لحظياً لكل الزبائن** |

### تفعيل التزامن السحابي (Firebase — مجاني، بلا خادم)
1. أنشئ مشروعاً على <https://console.firebase.google.com>.
2. فعّل **Firestore Database**.
3. أضف **تطبيق ويب** (`</>`) وانسخ قيم `firebaseConfig`.
4. الصقها في `config.ts` داخل `FIREBASE_CONFIG`.
   بمجرد ملء `apiKey` و`projectId` يتحوّل التطبيق تلقائياً للوضع السحابي.
5. (اختياري) اضبط قواعد Firestore للسماح بالقراءة للجميع والكتابة المحمية:
   ```
   match /store/prices {
     allow read: if true;
     allow write: if true;   // للتجربة فقط — قيّدها لاحقاً حسب حاجتك
   }
   ```

> الكتالوج (الأسماء/الصور/التصنيفات) يبقى محلياً في `data/products.ts`؛
> **الأسعار فقط** هي ما يُخزَّن ويتزامن. الكود في `lib/firebase.ts`، `lib/prices.ts`،
> و`context/PricesContext.tsx`.
